...
Installing with optional dependencies omitted, using an unsupported platform, or losing the selected payload makes the first delegation fail at `initialize` with a fixed safe category and any observed process outcome. Startup errors recognize `node-wrapper`, `dependency-missing`, `authentication`, `timeout`, and `network`; everything else remains `unknown`. Raw wrapper text remains on Host stderr; the provider neither probes a host CLI nor retries with one. An isolated wrapper fixture separately proves the native payload failure and absence of host fallback.
...
- **Authentication and account state remain native** — the Bundle supplies the CLI but does not create an account, log in, trust a project, or rewrite Codex settings; recognized startup authentication failures use the fixed `authentication` category, while unrecognized details retain the safe `unknown` fallback.
