...
Installing with optional dependencies omitted, using an unsupported platform, or losing the selected payload leaves provider registration dormant but makes the first delegation fail at the SDK startup boundary. The caller receives `query-start` with a fixed safe `dependency-missing`, `authentication`, `timeout`, or `network` category when recognized and `unknown` otherwise; the native payload error remains only on the internal cause chain and in the Provider's Host log. The provider neither probes a host CLI nor retries with one.
...
- **Authentication and account state remain native** — the Bundle supplies the CLI but does not create an account, log in, or rewrite Claude settings; recognized startup authentication failures use the fixed `authentication` category, while unrecognized details retain the safe `unknown` fallback.
