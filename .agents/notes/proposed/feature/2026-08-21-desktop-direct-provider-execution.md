...
A local image task first sends the original image and user question to the vision model. Pure recognition, OCR, or explanation can finish there. A coding task sends a structured observation, the original request, and a controlled local image reference to the coding model. The observation is marked as model analysis rather than image-ground-truth so the coding model does not treat an inference as certain. Resource-constrained systems rely on Ollama's `keep_alive` unloading and serial model loading rather than implementing a second model scheduler in the Host. The Host sets policy, avoids concurrent requests that would intentionally keep both 30B roles resident, and reports cold-load transitions to the UI.
