...
Stage one adds the executor registry, session field, and text-only Codex direct execution. Stage two adds Claude Code, streaming, cancellation, and structured errors. Stage three adds versioned preset migration and Web Search credential gating. Stage four enables separately verified image and attachment capabilities through official protocols. Stage five validates the installed application across local Qwen, Codex, Claude Code, proxies, offline mode, expired authentication, restoration, and no-fallback failures.
