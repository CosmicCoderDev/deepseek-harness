...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - The production Electron application reuses the same web client packages over an IPC fetch carrier. The [development POC](2026-08-15-electron-desktop-poc.md) temporarily supervises the browser carrier to validate lifecycle and security separately.
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Web (browser access) only; zero workspace dependencies (the registry arrives by structural injection); production Electron does not reuse it, while the development POC does temporarily |
...
| `DesktopApiClient` | the Electron shell | context-isolated IPC serialization round trip | shipped production desktop carrier; swaps only `doFetch` while retaining the base SSE parser and protocol contract |
