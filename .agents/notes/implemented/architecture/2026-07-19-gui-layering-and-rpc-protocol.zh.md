...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - 正式 Electron 应用经由 IPC fetch 载体复用同一套 web client 包。[开发验证版](2026-08-15-electron-desktop-poc.md)暂时监管浏览器载体，以便分别验证生命周期和安全性。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | Web（浏览器访问）专用；零 workspace 依赖（注册表经结构注入到达）；正式 Electron 不复用它，开发验证版暂时复用 |
...
| `DesktopApiClient` | Electron 壳 | 上下文隔离的 IPC 序列化往返 | 已发布的正式桌面载体；只替换 `doFetch`，继续复用基类 SSE 解析器与协议约定 |
