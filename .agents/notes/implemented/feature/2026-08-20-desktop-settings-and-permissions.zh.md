...
普通用户无需终端即可查看和配置桌面网络与子代理权限。主进程而非页面 JavaScript 继续承担校验与无限制访问确认的安全边界。现有代理偏好迁移时不会删除旧文件。权限变更会持久化并用于后续 provider 启动。ASAR 关闭期间安装包体积与文件数量较高，但已知可用的 preload 路径得以保留，重新评估 ASAR 的条件也已明确。
