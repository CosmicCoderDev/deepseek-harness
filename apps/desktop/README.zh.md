...
原生桌面设置页可以分别配置本地代码模型和本地视觉模型。页面从 Ollama `/api/tags` 读取已安装模型与磁盘占用，通过 `/api/show` 获取声明的图片能力，并从 `/api/ps` 显示当前常驻状态；能力字段缺失时保持**未知**，不会根据模型名称猜测。**测试图片识别**会把应用内置的一像素 PNG 通过 Ollama 的 OpenAI 兼容 `/v1/chat/completions` 端点发送，并展示实际助手回复。应用不会自动下载模型；当前所选模型名称也不会改写 Harness Provider 目录，因此在会话中使用视觉模型前，仍需在**设置 → 模型**中为该模型声明图片输入能力。
