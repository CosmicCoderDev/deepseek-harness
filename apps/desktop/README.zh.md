...
新会话开始前可以选择桌面执行方式：本地模型、自动选择、Codex 直接模式、Claude Code 直接模式或 Codex + Claude 预览。本地模式不会暴露两个云端子代理工具；直接模式失败时不会静默换用另一个执行者。由于 Agent 工具组在首轮消息后即固定，切换执行方式需要开始新会话。
...
Codex 与 Claude Code 各自提供**登录／重新登录**按钮。按钮会打开终端，并且只运行应用内置副本的固定官方登录命令；设置页渲染器不能传入命令或可执行文件路径。网页授权完成后，如果状态尚未变化，可以点击**刷新状态**。
Codex 与 Claude Code 状态由主进程缓存；并发刷新共用一次 CLI 检查，完成后的变化会推送到设置页，后台检查最快每三十秒一次。损坏或版本不支持的 `desktop-settings.json` 会先按时间戳保留，再安装安全默认值。权限变化后页面提供**立即重启 Host**；存在活动桌面请求时拒绝重启，新 Host 无法启动时回退到只读分析。
