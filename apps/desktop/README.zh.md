...
新会话开始前可以选择桌面执行方式：本地模型、自动选择、Codex 直接模式、Claude Code 直接模式、Codex + Claude 预览，或正式的“Codex 开发 → Claude 审核”工作流。正式工作流只暴露一个固定工具，不暴露两个独立 provider 工具：Codex 必须先执行，Claude Code 随后进行不修改文件的独立审核，任一阶段失败都会明确指出所属阶段；审核为“需修改”或“阻塞”时停止并等待用户决定，不会自动进入修复循环。本地模式不会暴露两个云端子代理工具；直接模式失败时不会静默换用另一个执行者。由于 Agent 工具组在首轮消息后即固定，切换执行方式需要开始新会话。

桌面子代理由封闭 Provider 注册表管理。设置页的安装、登录与状态卡片，以及权限映射和连通性测试，都从同一份 Provider 描述生成；渲染器不能指定任意模块路径或登录命令。
...
原生桌面设置页可以分别配置本地代码模型和本地视觉模型。页面从 Ollama `/api/tags` 读取已安装模型与磁盘占用，通过 `/api/show` 获取声明的图片能力，并从 `/api/ps` 显示当前常驻状态；能力字段缺失时保持**未知**，不会根据模型名称猜测。页面会明确显示是否可离线使用。所选视觉模型未安装时，**下载视觉模型**会先展示保守的体积预估并要求用户二次确认，然后才调用 Ollama 本机 `/api/pull`，绝不会静默下载。**测试图片识别**会把应用内置的一像素 PNG 通过 Ollama 的 OpenAI 兼容 `/v1/chat/completions` 端点发送，并展示实际助手回复。本地 Ollama 会话选择代码模型后，若某一轮进入的消息含图片，Host 会把该轮完整路由到配置的视觉模型；普通请求头会记录实际模型，后续工具步骤继续使用该视觉模型，下一轮纯文本任务则恢复代码模型。当前所选模型名称不会改写 Harness Provider 目录，因此在会话中使用视觉模型前，仍需在**设置 → 模型**中为该模型声明图片输入能力。
